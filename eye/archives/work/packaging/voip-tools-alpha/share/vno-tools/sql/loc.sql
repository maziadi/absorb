CREATE TABLE "<%= $vno_id %>_loc" (
  ...
  CONSTRAINT "<%= $vno_id %>_account_contact_idx" UNIQUE (username, domain, contact, callid)
);
DELETE FROM version WHERE table_name ='<%= $vno_id %>_loc';
INSERT INTO version VALUES ('<%= $vno_id %>_loc', (SELECT table_version FROM version WHERE table_name='location')) ;
